...
namespace app\modules\auth\v2\services;
...
use app\modules\auth\v2\domain\entities\AuthenticateAccessToken;
use app\modules\auth\v2\domain\entities\LoginAccessToken;
...
use app\modules\user\helpers\RoleHelper;
use Yii;
...
class LoginService
{
    ...
    {
        $user = UserRepository::getInstance()->getByEmailAndPassword($email, $password);

        if ($user === null || !$user->hasValidPassword($password)) {
    ...
    private function createNewLoginAttempt(): AbstractAccessToken
    {
        ...
        $token->ip = (string)yiiRequest()->userIP;
    ...
    private function addUser2faRole(): void
    {
        ...
        if (!$this->user->has2faRole()) {
            $role = $authManager->getRole(RoleHelper::ROLE_NEW_LOGIN_ATTEMPT->value);
